...
//Vector scan: the array to be scanned is stored
//in work-item private memory as uint4
inline uint2 scan2Inclusive(uint2 data2, __local uint *l_Data, uint size){
    //Level-0 inclusive scan
    data2.y += data2.x;
    
    //Level-1 exclusive scan
    uint val = scan1Inclusive(data2.y, l_Data, size / 2) - data2.y;

    return (data2 + (uint2)val);
}

inline uint2 scan2Exclusive(uint2 data2, __local uint *l_Data, uint size){
	return scan2Inclusive(data2, l_Data, size) - data2;
}

__kernel __attribute__((reqd_work_group_size(WORKGROUP_SIZE, 1, 1)))
void scanExclusiveSmall(
    __global uint2 *d_Dst,
    __global uint2 *d_Src,
    __local uint* l_Data,
    uint size
){
  	//Load data
    	uint2 idata2 = d_Src[get_global_id(0)];

    	//Calculate exclusive scan
    	uint2 odata2  = scan2Exclusive(idata2, l_Data, size);

    	//Write back
    	d_Dst[get_global_id(0)] = odata2;
}
